...
function capturarCompleto() {
  return new Promise((resolve) => {
    const container = document.getElementById("puntuacion-container");
    const botonesLaterales = document.querySelector(".contenedor-botones-laterales");
    ...
    wrapper.style.cssText = "display:inline-flex;flex-direction:row;align-items:center;gap:16px;background:white;padding:16px;position:absolute;left:-99999px;top:0;";

    const containerClone = container.cloneNode(true);
    containerClone.style.position = "static";

    const origInputs = container.querySelectorAll("input");
    const cloneInputs = containerClone.querySelectorAll("input");
    origInputs.forEach((inp, i) => {
      if (cloneInputs[i]) cloneInputs[i].value = inp.value;
    });

    const botonesClone = botonesLaterales ? botonesLaterales.cloneNode(true) : null;
    if (botonesClone) {
      botonesClone.style.cssText = "position:static;transform:none;display:flex;flex-direction:column;gap:40px;";
    }

    wrapper.appendChild(containerClone);
    if (botonesClone) wrapper.appendChild(botonesClone);
...
function descargarImagen() {
  capturarCompleto().then(canvas => {
    const inputVerde = document.querySelector("input.nombre-equipo-verde");
    const inputRojo = document.querySelector("input.nombre-equipo-rojo");
    const nombreVerde = inputVerde ? inputVerde.value.trim() : "EquipoVerde";
    const nombreRojo = inputRojo ? inputRojo.value.trim() : "EquipoRojo";
    const link = document.createElement("a");
    link.download = `${sanitizeFileName(nombreVerde)} vs ${sanitizeFileName(nombreRojo)}.png`;
...
function descargarPDF() {
  capturarCompleto().then(canvas => {
    ...
    const nombreVerde = inputVerde ? inputVerde.value.trim() : "EquipoVerde";
    const nombreRojo = inputRojo ? inputRojo.value.trim() : "EquipoRojo";
    pdf.save(`${sanitizeFileName(nombreVerde)} vs ${sanitizeFileName(nombreRojo)}.pdf`);
  });
}
...
function crearBotonTotal() {
  ...
  const wrapperCaptura = document.createElement("div");
  wrapperCaptura.style.position = "relative";
  ...
  `;
  botonCaptura.title = "Descargar";

  const menuDescarga = document.createElement("div");
  menuDescarga.id = "menu-descarga";
  menuDescarga.style.display = "none";

  const btnImg = document.createElement("button");
  btnImg.textContent = "Imagen (.png)";
  ...
  menuDescarga.appendChild(btnImg);
  menuDescarga.appendChild(btnPdf);

  botonCaptura.addEventListener("click", (e) => {
    e.stopPropagation();
    const visible = menuDescarga.style.display !== "none";
    menuDescarga.style.display = visible ? "none" : "flex";
  });
  ...
  wrapperCaptura.appendChild(botonCaptura);
  wrapperCaptura.appendChild(menuDescarga);
